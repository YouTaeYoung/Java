...
/*
 다형성(Polymorphism)
 	...
 	1.이경우 부모타입의 참조변수는 부모로부터 상속받은 멤버까지만
 		접근할 수 있다.
 	2.자식에서 오버라디이한 메서드가 우선 호출된다.
 	3.자식에서 새롭게 정의한 메법는 접근할 수 없다.
...
class Child extends Parent
{
	...
	//부모에서 정의한 메서드를 오버라이딩 하여 제정의한 메서드
	@Override
	void parentMethod()
	{
	...
	//오버로딩된 메서드(확장한 메서드)
	void parentMethod(int childMeber)
	{
		this.childMember = childMeber;
		System.out.println("Overlodaing: 자식에서 확인한 메소드" + ":parentMethod(param1)");
	}
}
...
public class E11Polymorphism
{
	public static void main(String[] args)
	{
		...
		//parent1.clidMember = 1; => 접근 불가 : 부모타입으로 자식멤버에 접근할수없다.
		...
		//자식쪽 멤버에 접근하는 방법1: 다운 캐스팅 후 접근한다.
