...
/*
 ...
 -추상클래스는 대부분 추상메서드를 포함하고 있으므로, 이를
 	상속받는 클래스는 반드시 추상메서드를 오버라디이 해야한다.
 -클래스의 상속관계에서 일종의 설계도 역할을 한다.
 -객체생성은 할 수 없으나 그의 참조변수, 상속 등의 모든 기능을
 	할 수 있다.
...
class PersonalInFoDTO
{
	...
	/*
	 getter /setter메서드 : DTO객체에서 private 으로 선언된  멤버변수의
	 	값을
	 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs
{
	...
	/*
	 상속을 통해 추상메서드를 초함하게 되었으므로 반드시 아래와 같이
	 오버라이딩 해야한다. 필수사항이며 하지 않을경우 에러가 발생한다.
	 */
	
	//전달받은 인자를 통해 DTO객체를 생헌한 후 객체배열을 추가한다.
	@Override
