...
public class E01WrapperClass2
{
	public static void main(String[] args)
	{
		...
		/*
		 실수형태으 ㅣ문자열을 실수로 변경할때는 parsrInt()를 사용할수 없다.
		 위와 동일한 예외가 발생한다.
		 */
		//System.out.println(Integer.parseInt(floatString)); => 예외발생
		
		System.out.println("실수형(float)형으로 변경 : " + Float.parseFloat(floatString));
		System.out.println("살수형(Double)형으로 변경 : " + Double.parseDouble(floatString));
		
		/*
		 codePonitAt() : 문자열에서 index에 해당하는 한 문자의 아스키코드
		 	값을 반환한다.
		 */
		...
		/*
		 isDigit() : 인자로 전달된 문자가 숫자인지 판ㄷ나한다. 숫자일때
		 true를 반환한다.
		 */
		...
		System.out.println("총 공 백 수 = " + whiteCount);
	}
}
