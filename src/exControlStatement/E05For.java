...
/*
 시나리오] for 문을 이용하여 1~10사이의 정수중 2의배수의 합을
 구하는 프로그램을 작성하시오.
 */
	//방법1:10번 반복에서 if문을 통해 2의배수를 찾은후 누적함
	int total = 0;
	for(int i=1; i<=10 ;i++) {
		if(i%2==0) {
			total += i;
	...
	//방법2: if문 없이 2씩 증가하면서 누적함
	total = 0;
	for(int i=0 ; i<=10; i+=2) {//복합대입 연사자 사용가능함.
		total += i;
	}
...
//	}
	...
	/*
	for문의 최기식에서 선언된 변수 j는 for문이 종료되는 순간
 ...
 */
	...
	for(int dan = 2; dan <= 9; dan++){//2단부터 9단까지 반복
		for(int su = 1; su <= 9; su++) { //1~까지 반복
			System.out.printf("%2d*%2d=%2d ",dan,su,(dan*su) );
		//내가한 코드 System.out.print(+ dan + " *" + su + " = "+ (dan*su));
		}
		System.out.println();
	}
