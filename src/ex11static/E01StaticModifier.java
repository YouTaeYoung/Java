...
/*
 static veriable (정적변수)
 -멤버변수 앞에 static 키워드가 붙으면 정적변수가 된다.
 -정적변수는 JAM(자바가상머신)에 의해 프로그램이 시작되는 시점에
 	초기화 된다.
...
class MyStatic
{
	...
	int instanceVar;
	/*
	 정적멤버변수] 객체를 생성할 필요없이 클래스명으로 바로 접근할 수 있따.
	...
	/*
	 정적 멤버메소드] 정적 메소드 안에서는 인스턴스형 멤버를 사용할수없다.
	 	정적 멤버만 접근할 수 있따.(*main 메소드에서는 static으로 선언된
