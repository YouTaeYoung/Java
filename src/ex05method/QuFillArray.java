...
public class QuFillArray
{
	public static void main(String[] args)
	{
		...
		//arr2 배열의 시작인데스와 마지막인덱스를 변수로 지정한다.
		int indexStart = 0 ;
		int indexEen = arr2.length - 1; //9 => 배열의 크기가 결정되면 length를 사용하는게 좋다.
		...
		for(int i = 0 ; i < arr1.length ; i++ )
		{
			...
			if(inputNum % 2 == 0)
			{
				//짝수일떄 : 배열의 끝에서부터 채워나간다.
				//값을 채운후 인데스는  1 차감한다.
				arr2[indexEen--] = inputNum;
			}
			else
			{
				//홀수일때 : 배열의 처음부터 채워나간다.
