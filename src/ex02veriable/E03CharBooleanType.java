...
public class E03CharBooleanType {
	...
	/*
	 ...
	 '(sungle quotation)으로 감싸서 표현한다.
	 만약 "(double quotation)으로 감싸면 문자열로 인식하므로
	 String으로 선언해야 한다.
	 */
		char ch1 = '가';
		//char chStr = '가나다라'; //에로발생. 문자열이므로 char로 선언불가능.
		String chStr = "가나다라"; //String 으로 선언해야함.
		...
		/*
		 boolean형 : ture 혹은 false 두가지의 값만 가질수 잇는 자료형으로
		 산술연산 (+,-)이나 비교연산 (<.<=)에서는 사용할수 없고
		 논리연산에서 사용할 수 있다.
